...
def build_file_paths(roots, suffixes):
    ...
    for group_name in group_names:
        if not isdir(join(pred_base, group_name)) or group_name == '.ipynb_checkpoints':
            continue
        ...
        gt_paths += list(map(lambda pred_name: join(gt_base, group_name, pred_name[:-len(pred_suffix)] + gt_suffix),
                             group_pred_names))
...
def get_loader(roots, suffixes, batch_size, num_thread, pin=True):
    ...
    data_loader = data.DataLoader(dataset=dataset, shuffle=False, batch_size=batch_size, num_workers=num_thread,
                                  pin_memory=pin)
    return data_loader