...
def calc_smeasure(gt, pred):
    ...
    def centroid(y):
        ...
        hw = int(torch.round(torch.sum(torch.sum(y, axis=0) * torch.from_numpy(np.array(range(1, 1 + w))).cpu()) / total))
        hh = int(torch.round(torch.sum(torch.sum(y, axis=1) * torch.from_numpy(np.array(range(1, 1 + h))).cpu()) / total))
